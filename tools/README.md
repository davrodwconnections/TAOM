...
| `analyze_item_values.py` | **Read-only item-value skew report** for the decision #318 asks for. Per armor `Type`, the median/p90/max of the governing armor stat for LOTRLOME vs vanilla SandBoxCore, with the ratio; explicit-`value=` coverage on both sides (LOTRLOME ships **0%**, so every price is derived); and the items whose stat sits above the highest vanilla item of the same type. **It deliberately does NOT reimplement `DefaultItemValueModel`** — guessing an engine formula would produce authoritative-looking prices nobody can check, and the report says "not supplied" rather than inferring. Pass `--values-csv id,value` with values read from a running game to get the real price ratio alongside the stat ratio. Regex parse, not ElementTree: several shipped armor files carry fragments a strict parse rejects, and one bad file must not silence the other few thousand items. The two trees take separate path overrides because LOTRAOM ships as its own copy of the game, so the Armory and vanilla can live in different installs. Reports to `tools/reports/item-values/`. | `--armory-path`, `--vanilla-path`, `--game-dir`, `--values-csv`, `--json`, `--stdout` |
...
`setup-dev-env.ps1` sets it. 30 tools honour it: 26 resolve it themselves, and `analyze_armor_balance.py`,
